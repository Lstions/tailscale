// Copyright (c) Tailscale Inc & contributors
// SPDX-License-Identifier: BSD-3-Clause

// Code generated by "stringer -type=discoPingPurpose -trimprefix=ping"; DO NOT EDIT.

package magicsock

import "strconv"

func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
	var x [1]struct{}
	_ = x[pingDiscovery-0]
	_ = x[pingHeartbeat-1]
	_ = x[pingCLI-2]
	_ = x[pingHeartbeatForUDPLifetime-3]
	_ = x[pingRecovery-4]
	_ = x[pingCandidateVerification-5]
	_ = x[pingPathQuality-6]
}

const _discoPingPurpose_name = "DiscoveryHeartbeatCLIHeartbeatForUDPLifetimeRecoveryCandidateVerificationPathQuality"

var _discoPingPurpose_index = [...]uint8{0, 9, 18, 21, 44, 52, 73, 84}

func (i discoPingPurpose) String() string {
	idx := int(i) - 0
	if i < 0 || idx >= len(_discoPingPurpose_index)-1 {
		return "discoPingPurpose(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _discoPingPurpose_name[_discoPingPurpose_index[idx]:_discoPingPurpose_index[idx+1]]
}
